...
function createArea(title, url, image, text, opacity, display) {
	...
	let areaCenter = isHorizontal
		? [diskInc * 0.25 * width, 0.5 * height]
		: [0.5 * width, diskInc * 0.25 * height];
	...
	arrow.setAttributeNS(
		null,
		'points',
		'0.5 0.82 0.5 18.74 17.81 9.87 0.5 0.82'
	);
...
function createScrollArrows() {
	if (isHorizontal) {
		...
		const scrollArrowLeft = document.createElementNS(
			'http://www.w3.org/2000/svg',
			'svg'
		);
		...
		arrowLeft.setAttributeNS(
			null,
			'points',
			'68,19.1 29.7,49.1 68,79.1 69.2,77.6 32.9,49.1 69.2,20.6 68,19.1 '
		);

		scrollArrowLeft.style.left = 0.04 * width - 20 + 'px';
		scrollArrowLeft.style.top = 0.5 * height - 40 + 'px';
		...
		const scrollArrowRight = document.createElementNS(
			'http://www.w3.org/2000/svg',
			'svg'
		);
		...
		arrowRight.setAttributeNS(
			null,
			'points',
			'30.4,20.6 66.7,49.1 30.4,77.6 31.6,79.1 69.9,49.1 31.6,19.1 30.4,20.6 '
		);

		scrollArrowRight.style.right = 0.04 * width - 20 + 'px';
		scrollArrowRight.style.top = 0.5 * height - 40 + 'px';
		...
		const scrollArrowTop = document.createElementNS(
			'http://www.w3.org/2000/svg',
			'svg'
		);
		...
		const arrowTop = document.createElementNS(
			'http://www.w3.org/2000/svg',
			'polygon'
		);
		arrowTop.setAttributeNS(
			null,
			'points',
			'22.4,64.1 50.8,36.5 79.3,64.1 80.8,63.2 50.8,34.1 20.8,63.2 22.4,64.1 '
		);

		scrollArrowTop.style.left = 0.5 * width - 50 + 'px';
		scrollArrowTop.style.top = 0.04 * height - 20 + 'px';
		...
		const scrollArrowBottom = document.createElementNS(
			'http://www.w3.org/2000/svg',
			'svg'
		);
		...
		const arrowBottom = document.createElementNS(
			'http://www.w3.org/2000/svg',
			'polygon'
		);
		arrowBottom.setAttributeNS(
			null,
			'points',
			'23.1,34.1 51.5,61.7 80,34.1 81.5,35 51.5,64.1 21.5,35 23.1,34.1 '
		);

		scrollArrowBottom.style.left = 0.5 * width - 50 + 'px';
		scrollArrowBottom.style.bottom = 0.04 * height - 20 + 'px';
...
disks.push(
	createArea(
		'AUTRES TRAVAUX',
		'https://github.com/benoitclement',
		'images/my-github.png',
		'Le code de tous mes projets sur ma page github.',
		(opacity = false),
		(display = true)
	)
);
disks.push(
	createArea(
		'HOTEL PARIMIS',
		'https://parimis.benoitclement.fr',
		'images/parimis.png',
		"Site vitrine 100% responsive pour un hôtel de luxe d'après cahier des charges. HTML5, PostCSS, Javascript, Gulp, Git.",
		(opacity = true),
		(display = false)
	)
);
disks.push(
	createArea(
		...
		'../cv2019',
		'images/mind-map.png',
		"Outil de mise en forme façon carte heuristique de contenus interconnectés de type texte ou image. Le but est à terme de produire une interface complète WYSIWYG pour la mise en relation d'informations.",
		(opacity = true),
		(display = false)
	)
);
...
disks.push(
	createArea(
		'MATCH-3 BOT',
		'https://github.com/benoitclement/match3-bot',
		'images/match3bot.png',
		'Programme "bot" / solutionneur de puzzle game type match-3 (ex. Candy Crush). Analyse en temps réel l\'affichage écran avant de décider du meilleur mouvement selon un système de priorités. Réalisé avec Python, OpenCV, Numpy.',
		(opacity = false),
		(display = true)
	)
);
disks.push(
	createArea(
		'GUESS THE WORD',
		'https://github.com/benoitclement/guess-a-word',
		'images/guess-the-word.png',
		'Programme de jeu établissant une liste de 10 mots à deviner avec 3 essais pour chacun. Le joueur forme les mots en glissant-déplaçant les lettres. Python, TKinter.',
		(opacity = false),
		(display = true)
	)
);

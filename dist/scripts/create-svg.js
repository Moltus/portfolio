...
function getInkInfo() {
  let posX = Math.floor(Math.random() * (width - 100)) + 50;
  let posY = Math.floor(Math.random() * (height - 100)) + 50;
...
let interval = setInterval(showRndCircle, 100);
...
function showRndCircle() {
  if (inkDropsIds.length === 0) {
    ...
    document.getElementById(rndId).style.filter = 'blur(20px)';
